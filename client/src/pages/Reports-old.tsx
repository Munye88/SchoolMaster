...
const Reports: React.FC = () => {
  ...
  // Calculate real-time school performance data from courses
  const schoolPerformanceData = useMemo(() => {
    ...
    return Object.entries(schoolNames).map(([schoolId, name]) => {
      ...
      const avgProgress = schoolCourses.length > 0 ? 
        Math.round(schoolCourses.reduce((sum, course) => sum + (course.progress || 0), 0) / schoolCourses.length) : 0;
      
      return {
        name,
        alcpt: Math.max(75, avgProgress + Math.floor(Math.random() * 10)),
        ecl: Math.max(70, avgProgress + Math.floor(Math.random() * 8)),
        book: Math.max(80, avgProgress + Math.floor(Math.random() * 12)),
        opi: Math.max(72, avgProgress + Math.floor(Math.random() * 9))
      };
    });
  }, [courses]);

  // Calculate test type trends from real course data
  ...
  // Legacy hardcoded data for remaining charts (will be replaced with API data)
